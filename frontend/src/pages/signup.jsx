...
export const Signup = () => {
  ...
  const googleAuth = () => {
    window.open("https://login-seven-pink.vercel.app/auth/google/callback", "_self");
  };
  ...
  const onOTPVerify = async () => {
    ...
    window.confirmationResult
      ...
      .then(async (res) => {
        setLoading(false);
        try {
          const response = await axios.post("https://login-seven-pink.vercel.app/auth/otp-verify", {
            phoneNumber,
            firstName,
            lastName,
          });
            ...
            <div className="pt-4">
              {showOTP ? (
                ...
                <>
                  ...
                  {error && <div className="text-red-500 pt-2">{error}</div>}
                  <div className="pt-4">
                  <Button onClick={handleOTPSignup} label={"Send code via SMS"}>
                    {loading && <CgSpinner size={20} className="mt-1 animate-spin" />}
                  </Button>
                  </div>
                </>
